...
public static class MatchFileWriter
{
    ...
    public static void WriteBallRecordsWithTotalRunsAndWickets(List<BallResult> results, string path)
    {
        ...
        using var writer = new StreamWriter(path);
        writer.WriteLine(
            "Bowler Name, Batter 1 Name, Batter 2 Name, Result - Runs, Result - Wickets, Total Runs, Total Wickets");
        foreach (var r in summary)
            writer.WriteLine(
                $"{r.ballResult.Bowler},{r.ballResult.Batter1},{r.ballResult.Batter2},{r.ballResult.Runs},{r.ballResult.Wickets},{r.Runs},{r.Wickets}");
    }
}